...
-- Среднее значение оценок между 50 и 75 у студентов
SELECT student_id, ROUND(AVG(score),1) AS avg_score
FROM grades
GROUP BY student_id HAVING ROUND(AVG(score),1) BETWEEN 50 AND 75 -- фильтр после выборки
ORDER BY student_id;

-- просматривать пересекающиеся записи (INTERSECT) и извлекать отличающиеся данные (EXCEPT)
-- ARRAY_AGG и GROUP BY собирает поля в массив
